...
# Box 데이터
box_data = {
    'P_A+': [(-0.17, -0.07),(0.17, 0.07),(0.17, 0.14),(-0.17, 0.14)],
    'P_A': [(-0.34, -0.16), (0.34, -0.16), (0.32, 0.32), (-0.32, 0.32)],
    'D_A+': [(0.82, -0.04), (0.82, 0.08),(0.70, 0.1), (0.71, -0.05)  ],
    'D_A': [(0.86, -0.09), (0.85, 0.18), (0.62, 0.27), (0.64, -0.13) ]
}

def create_cross_section_plots(df_long, cross_direction):
    """크로스섹션 플롯 생성 (matplotlib로 PNG 저장용) - 하나의 그래프에 양방향 표시"""
    ...
    try:
        ...
        if cross_direction == "가로 (0°-180°)":
            ...
            # 하나의 그래프에 양방향 표시
            ax.plot(df_0['Theta'], df_0['Luminance'], 'r-o', linewidth=2, markersize=5, 
                   label='0° direction (우측)', alpha=0.8)
            ax.plot(df_180['Theta'], df_180['Luminance'], 'b-s', linewidth=2, markersize=5, 
                   label='180° direction (좌측)', alpha=0.8)
            
            ax.set_xlabel('Theta (degrees)')
            ax.set_ylabel('Luminance')
            ax.set_title('가로 방향 휘도 프로파일 (0°-180°)')
            ...
            # 하나의 그래프에 양방향 표시
            ax.plot(df_90['Theta'], df_90['Luminance'], 'g-o', linewidth=2, markersize=5, 
                   label='90° direction (상단)', alpha=0.8)
            ax.plot(df_270['Theta'], df_270['Luminance'], 'm-s', linewidth=2, markersize=5, 
                   label='270° direction (하단)', alpha=0.8)
            
            ax.set_xlabel('Theta (degrees)')
            ax.set_ylabel('Luminance')
            ax.set_title('세로 방향 휘도 프로파일 (90°-270°)')
            ax.grid(True, alpha=0.3)
            ax.legend()
